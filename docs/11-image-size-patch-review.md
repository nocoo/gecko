...
当天 npm 最新版本为 `2.0.4`，两份 OSV 公告均标记最后受影响版本为 `2.0.2`。本次将 dashboard 的 `image-size` override 固定到 `2.0.4`，移除旧的 Bun 补丁及两条例外。原有三份零长度畸形输入仍由 CJS / ESM 的公开入口、直接解析器执行，保留每个子进程 2 秒超时；并增加正常 1×1 PNG 的两个模块系统对照测试。安全契约是进程必须在超时前结束：畸形输入要么给出有限宽高，要么抛出非空错误。macOS 与 Linux 对同一 2.0.4 可能一个返回尺寸、一个 `TypeError`，两者都算终止，不能把其中一种文案当成唯一合法结果。安全 gate 继续先检查解析器，再扫描 OSV 和 Gitleaks。
