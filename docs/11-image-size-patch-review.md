...
扫描器按锁文件中的原始 npm 版本匹配，不识别本仓库的源码补丁。当前例外仅用于已回补这两项问题的安装结果。

## 复核证据

- [既有补丁](../apps/web-dashboard/patches/image-size@2.0.2.patch)覆盖 CJS / ESM 的公开入口、直接解析器及分发文件；零长度字段使偏移量至少前进 8 字节。
...
- 对当前已安装补丁执行[现有回归测试](../apps/web-dashboard/src/__tests__/image-size-security.test.ts)，6 个 CJS / ESM 用例全部通过；每个用例检查公开入口和直接解析器。畸形 ICNS / HEIF 有限返回尺寸，JXL 有限返回输入结束错误，结果与预期一致。

## 后续检查

[安全检查入口](../apps/web-dashboard/scripts/gate-security.ts)现在先执行这组现有回归测试，补丁行为不通过则立即失败；随后仍运行完整 OSV 与 Gitleaks 检查。两个已有例外在上述复核后限期续至 **2026-10-08**，其他依赖和公告照常检查。
