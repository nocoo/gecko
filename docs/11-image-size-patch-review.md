...
当天 npm 最新版本为 `2.0.4`，两份 OSV 公告均标记最后受影响版本为 `2.0.2`。本次将 dashboard 的 `image-size` override 固定到 `2.0.4`，移除旧的 Bun 补丁及两条例外。原有三份零长度畸形输入仍由 CJS / ESM 的公开入口、直接解析器执行，保留每个子进程 2 秒超时；并增加正常 1×1 PNG 的两个模块系统对照测试。安全契约是进程必须在超时前结束：畸形 ICNS / HEIF 有限返回尺寸，畸形 JXL 返回输入结束错误。这与补丁时代的实测一致，不是 `TypeError` 拒绝。安全 gate 继续先检查解析器，再扫描 OSV 和 Gitleaks。
